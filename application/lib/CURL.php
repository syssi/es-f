...
class cURL {
  ...
  public function __construct( $url='', $opts=array() ) {
    if ($url) $this->addSession($url, $opts);
  }
  ...
  public function addSession( $url, $opts=FALSE ) {
    $key = count( $this->sessions );
    $this->sessions[$key] = curl_init( $url );
    if ($opts) $this->setOptArray( $opts, $key );
    return $key;
  }
  ...
  * @param $key int, session key to set option for
  */
  public function setOptArray( $options, $key=0 ) {
    foreach($options as $option=>$value) $this->setOpt($option, $value, $key);
  }
