...
class dBug {
  ...
  private $xmlAttrib;
  private $xmlName;
  private $arrType=array("array","object","resource","boolean");
  private $bInitialized = FALSE;
  private $bCollapsed = FALSE;
  private $sVarName = NULL;
  private $arrHistory = array();

  //constructor
  function dBug($var,$options=array()) {
    //include js and css scripts
    echo $this->initJSandCSS();
    $this->bCollapsed = isset($options['collapse']) AND $options['collapse'];
    $this->sVarName = isset($options['name']) ? $options['name'] : NULL;
    if (isset($options['forcetype']) AND
        //array of variable types that can be "forced"
        in_array($options['forcetype'],array('array','object','xml')))
  ...
  function getVariableName() {
    ...
    if (isset($arrFile)) {
      $arrLines = file($arrFile["file"], FILE_IGNORE_NEW_LINES);
  ...
  function checkType($var) {
    switch(TRUE) {
      ...
      case is_bool($var):
        ...
        break;
      default:
        echo ($var==='') ? '[empty string]' : htmlspecialchars($var);
        break;
    }
  }
...
*/
  ...
  <style type="text/css">
    ...
    table.dBug_xml { font-family:Verdana, Arial, Helvetica, sans-serif; color:#000000; font-size:12px; }

    table.dBug_array td,
    table.dBug_object td,
    table.dBug_resource td,
    table.dBug_resourceC td,
    table.dBug_xml td { vertical-align:top; }

    .dBug_arrayHeader,
    .dBug_objectHeader,
    .dBug_resourceHeader,
    .dBug_resourceCHeader,
    .dBug_xmlHeader { font-weight:bold; color:#FFFFFF; cursor:pointer; }

    .dBug_arrayKey,
    .dBug_objectKey,
    .dBug_xmlKey { cursor:pointer; }
