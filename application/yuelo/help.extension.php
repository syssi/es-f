<?php
/**
 * Yuelo Extension help
 *
 * @usage
 * @code
 * Template:
 *   {help:"WhatsThis"}
 *
 * Output:
 *   &lt;a href="?module=help&action=popup&topic=WhatsThis"&gt;&lt;img src="image..."&gt;&lt;/a&gt;
 * @endcode
 *
 * @author     Knut Kohl <knutkohl@users.sourceforge.net>
 * @copyright  2006-2011 Knut Kohl
 * @license    GNU General Public License http://www.gnu.org/licenses/gpl.txt
 * @version    $Id: v2.4.1-62-gb38404e 2011-01-30 22:35:34 +0100 $
 * @revision   $Rev$
 */
class Yuelo_Extension_Help extends Yuelo_Extension {

  /**
   * Yuelo Extension help
   *
   * @param string $topic Help topic Id
   * @return string
   */
  public static function Process() {
    list($topic) = self::params(func_get_args());
    $params = $aparams = array('t' => $topic);
    $aparams['a'] = 1;
    return '<span class="helplink">'
         . '<a href="'
         . Core::URL(array('module'=>'help', 'action'=>'topic', 'params'=>$params))
         . '" onclick="return AjaxPopupWindow(\''
         . Core::URL(array('module'=>'help', 'action'=>'topic', 'params'=>$aparams))
         . '\',480,320)">'
         . '<img style="cursor:help" src="layout/images/help-topic.gif">'
         . '</a>'
         . '</span>';
  }

}