...
// defaultDomain = string, default domain for cookies; default is current domain
// defaultPath = string, default path for cookies; default is '/'
function Cookiemanager(name, defaultPrefix, defaultExpiration, expirationUnits, defaultDomain, defaultPath) {
  // remember our name
  this.name = name;
  // get the default cookie prefix handeled by this cookie manager
  this.prefix = ((defaultPrefix) ? defaultPrefix : 'c') + '_';
  ...
  this.defaultDomain = (defaultDomain)
                     ? defaultDomain
                     : ( (document.domain.search(/[a-zA-Z]/) == -1) // IP
                       ? document.domain
...
Cookiemanager.prototype.getDocumentCookies = function() {
  ...
  for(var i=0;i < len;i++) {
    ...
    pair = cookie.split('=');
    if (pair[0].substr(0, this.prefix.length) == this.prefix) {
      // use the cookie name as the property name and value as the value
      this.cookies[pair[0].substr(this.prefix.length)] = pair[1];
    }
  }
};
...
Cookiemanager.prototype.setDocumentCookies = function() {
  var expires; var domain; var path; var cookies;
  for(var name in this.cookies) {
    ...
    cookies = this.prefix + name + '=' + this.cookies[name] + '; expires=' + expires +
